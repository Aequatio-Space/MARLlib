...
class ReplayBuffer(object):
    ...
    def sample_trajwise_observation(self, batch_size, sample_type=None):
        ...
        if sample_type == 'only_state':
            obses = torch.as_tensor(self.observes_traj[idxs, :, :self.state_idx_dict[self.env_name]],
                                    device=self.device).float()  # [bs, ts, dim]
            ...
            obses = torch.as_tensor(np.concatenate([pure_obs, init_state], axis=-1),
                                    device=self.device).float()  # [bs, ts, dim]
        else:
    ...
    def sample(self, batch_size, discount, idxs=None):
        ...
        if self.sample_type == 'only_state':
            ...
            next_obses = torch.as_tensor(np.concatenate([pure_next_obs, init_next_state], axis=-1),
                                         device=self.device).float()
        else:
...
class HindsightExperienceReplayWrapperVer2(object):
    ...
    def sample(self, batch_size, *args, **kwargs):
        ...
        obses, actions, rewards, next_obses, discounts, dones = self.replay_buffer.sample(None, idxs=indices, *args,
                                                                                          **kwargs)  # batch_size*0.2
        ...
        obs_dict, next_obs_dict = map(self.env.convert_obs_to_dict,
                                      (obses, next_obses))  # TODO: convert should address the batch inputs
        ...
        if num_future_goals > 0:
            ...
            future_obs_idxs = np.array([
                self._idx_to_future_obs_idx[ids][next_obs_idxs[i]] if self._idx_to_future_obs_idx[ids].shape[
                                                                          0] != 0 else ids  # original next_obs idx
            ...
            future_next_obses_dict = self.env.convert_obs_to_dict(
                future_next_obses)  # TODO: convert should address the batch inputs
            ...
            if self.env_name in ['AntMazeSmall-v0', "PointUMaze-v0", "PointSpiralMaze-v0", "PointNMaze-v0"]:
                relabeled_reward = self.env.compute_reward(next_obs_dict['achieved_goal'][-num_future_goals:], goal,
                                                           sparse=True)
            elif self.env_name in ['sawyer_peg_push', 'sawyer_peg_pick_and_place']:
                relabeled_reward = self.env.compute_reward(
                    np.concatenate([next_obs_dict['observation'][-num_future_goals:], goal], axis=-1))
            else:
                relabeled_reward = self.env.compute_reward(
                    np.concatenate([next_obs_dict['achieved_goal'][-num_future_goals:], goal], axis=-1))

            # Transform back to ndarrays
            relabeled_obs, relabeled_next_obs = map(self.env.convert_dict_to_obs,
                                                    (obs_dict, next_obs_dict))  # [batch_size]
    ...
    def _store_episode(self):
        ...
        for transition_idx, transition in enumerate(self.episode_transitions):
            ...
            if current_transition_idx + 1 + remained_timesteps_in_current_episode > self.replay_buffer.capacity:
                ...
                future_obs_indices_rear = np.arange(current_transition_idx + 1,
                                                    self.replay_buffer.capacity)  # e.g. [10]
                future_obs_indices_front = np.arange(0, remained_timesteps_in_current_episode -
                                                     future_obs_indices_rear.shape[0])  # [39]

                self._idx_to_future_obs_idx[current_transition_idx] = np.concatenate(
                    [future_obs_indices_rear, future_obs_indices_front], axis=0)
            else:
                self._idx_to_future_obs_idx[current_transition_idx] = np.arange(current_transition_idx + 1,
                                                                                current_transition_idx + 1 + remained_timesteps_in_current_episode)
