...
class OUTPACEAgent(object):
            ...
            if self.aim_input_type == 'default':
                ...
                if self.inv_weight_curriculum_logit_type == 'disc':
                    if self.normalize_f_obs:
                        aim_outputs = self.aim_discriminator.forward(
                            self.normalize_obs(torch.cat([obs_dict['achieved_goal'], obs_desired_goal], dim=-1),
                                               env_name=self.env_name)).squeeze()  # [bs, 1]-> [bs]
                    else:
                        aim_outputs = self.aim_discriminator.forward(
                            torch.cat([obs_dict['achieved_goal'], obs_desired_goal],
                                      dim=-1)).squeeze()  # [bs, 1]-> [bs]
                elif self.inv_weight_curriculum_logit_type == 'reward':
                    aim_outputs = self.compute_aim_reward(
                        torch.cat([obs_dict['achieved_goal'], obs_desired_goal], dim=-1)).squeeze()  # [bs, 1]-> [bs]
                else:
    ...
    def sample_idx_by_aim_outputs(self, obs: torch.Tensor, goal_env, n_sample, topk=None, return_w_prob=False):
        ...
        if self.inv_weight_curriculum_type == 'softmin':
            ...
            logits = ((aim_outputs - aim_outputs_min) / (
    ...
    def sample_randomwalk_goals(self, obs, ag, episode, env, replay_buffer, num_candidate=5, random_noise=2.5,
                                uncertainty_mode='f'):
        ...
        if uncertainty_mode == 'nml' and self.use_meta_nml:
            start = time.time()
            classification_probabilities = self.get_prob_by_meta_nml(candidate_goal, episode,
                                                                     replay_buffer=replay_buffer, goal_env=env)
            ...
            prob = F.softmax(torch.from_numpy(
                classification_probabilities[uncertain_indices] / self.meta_nml_temperature).float().to(self.device),
                             dim=0)
            dist = torch.distributions.Categorical(probs=prob)
            ...
            indices = self.sample_idx_by_aim_outputs(torch.as_tensor(aim_obs, device=self.device).float(), env, 1,
                                                     topk=aim_obs.shape[0])
    ...
    def update_aim_discriminator(self, aim_disc_replay_buffer, goal_env):
        ...
        if self.env_name in ['sawyer_door', 'sawyer_peg']:
            ...
            if self.aim_input_type == 'default':
                ...
                policy_states = torch.cat([get_object_states_only_from_goal(self.env_name, obs_dict['achieved_goal']),
                                           get_object_states_only_from_goal(self.env_name, obs_dict['desired_goal'])],
                                          dim=-1)  # s, s_g
                policy_next_states = torch.cat(
                    [get_object_states_only_from_goal(self.env_name, next_obs_dict['achieved_goal']),
                     get_object_states_only_from_goal(self.env_name, next_obs_dict['desired_goal'])],
                    dim=-1)  # s', s_g
                target_states = torch.cat([get_object_states_only_from_goal(self.env_name, next_obs_dict[
                    'desired_goal']) + torch.from_numpy(np.random.normal(scale=scale,
                                                                         size=get_object_states_only_from_goal(
                                                                             self.env_name, next_obs_dict[
                                                                                 'desired_goal']).shape)).float().to(
                    self.device), get_object_states_only_from_goal(self.env_name, next_obs_dict['desired_goal'])],
                                          dim=-1)  # s_g, s_g
            ...
            if self.aim_input_type == 'default':
                ...
                policy_next_states = torch.cat([next_obs_dict['achieved_goal'], next_obs_desired_goal],
                                               dim=-1)  # s', s_g
                target_states = torch.cat([next_obs_desired_goal + torch.from_numpy(
                    np.random.normal(scale=scale, size=next_obs_desired_goal.shape)).float().to(self.device),
                                           next_obs_desired_goal], dim=-1)  # s_g, s_g
    ...
    def update(self, replay_buffer, randomwalk_buffer, aim_disc_replay_buffer, step, goal_env=None, goal_buffer=None):
            ...
            if step == self.num_seed_steps // 2:
                ...
                if self.use_meta_nml:
                    ...
                    if self.meta_nml_train_on_positives:
                        train_data = self.sample_meta_test_batch(self.meta_train_sample_size,
                                                                 replay_buffer=replay_buffer, goal_env=goal_env)
                    else:
                        train_data = self.sample_negatives(replay_buffer, goal_env, self.meta_train_sample_size)
                    test_data = self.sample_meta_test_batch(self.meta_test_sample_size, replay_buffer=replay_buffer,
                                                            goal_env=goal_env)
                    ...
                    self.meta_nml_results = self.meta_nml.train(train_data, test_data,
                                                                batch_size=self.meta_task_batch_size,
                                                                accumulation_steps=self.accumulation_steps,
                                                                num_epochs=num_epochs, test_strategy=self.test_strategy,
                                                                test_batch_size=self.meta_test_batch_size,
        ...
        if self.use_aim:
            ...
            if self.use_meta_nml and step % self.meta_nml_train_every_k == 0:
                ...
                if self.meta_nml_train_on_positives:
                    train_data = self.sample_meta_test_batch(self.meta_train_sample_size, replay_buffer=replay_buffer,
                                                             goal_env=goal_env)
                else:
                    train_data = self.sample_negatives(replay_buffer, goal_env, self.meta_train_sample_size)
                test_data = self.sample_meta_test_batch(self.meta_test_sample_size, replay_buffer=replay_buffer,
        ...
        if randomwalk_buffer is None or self.randomwalk_method == 'rand_action':
            ...
            obs, action, extr_reward, next_obs, discount, dones = utils.sample_mixed_buffer(replay_buffer,
                                                                                            randomwalk_buffer,
                                                                                            self.batch_size,
                                                                                            self.discount)
        ...
        if self.use_aim:
            with torch.no_grad():
                ...
                if self.env_name in ['sawyer_door', 'sawyer_peg']:
                    if self.aim_input_type == 'default':
                        self.intr_reward = self.compute_aim_reward(torch.cat(
                            [get_object_states_only_from_goal(self.env_name, next_obs_dict['achieved_goal']),
                             get_object_states_only_from_goal(self.env_name, next_obs_desired_goal)], dim=-1))

                else:
                    if self.aim_input_type == 'default':
                        self.intr_reward = self.compute_aim_reward(
                            torch.cat([next_obs_dict['achieved_goal'], next_obs_desired_goal], dim=-1))
        ...
        Q1, Q2, critic_loss, offline_rl_dict, discor_dict = self.update_critic(obs, action, reward, next_obs, discount,
                                                                               dones, step)

        if step % self.actor_update_frequency == 0:
            self.actor_loss, self.alpha_loss, self.actor_log_prob = self.update_actor_and_alpha(obs, step,
                                                                                                offline_rl_dict=offline_rl_dict)
        ...
        if self.use_aim:
            ...
            logging_dict.update(
                {'aim_wgan_loss': self.wgan_loss.detach().cpu().numpy()})  # it should be decreases as it converges?
            logging_dict.update({
                'aim_graph_penalty_loss': self.graph_penalty.detach().cpu().numpy()})  # it should be near zero to satisfy lipschitz constraint?
