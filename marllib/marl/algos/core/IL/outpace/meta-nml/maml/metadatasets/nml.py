...
class NML(MetaDataset):
    ...
    def __init__(self, train_data, test_data=None, points_per_task=1, test_strategy='all', test_batch_size=0,
        ...
        assert num_tasks <= self.num_classes * len(
            self._train_inputs), "Cannot have more tasks than (num classes) x (total training points)"
...
def get_dataset_from_file(
    ...
    replay_pool_states = np.load(filename)[-int(replay_pool_size):] if not from_beginning else np.load(filename)[
                                                                                               :int(replay_pool_size)]
    print(f"Replay pool has {len(replay_pool_states)} states, each trajectory clipped at {clip_length}")

    if clip_length < 100:
        replay_pool_states = replay_pool_states.reshape(-1, 100, 2).transpose((0, 2, 1))[..., :clip_length].transpose(
            (0, 2, 1)).reshape(-1, 2)
