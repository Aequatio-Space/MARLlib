...
class CustomMetaDataLoaderForHydraMultiProcess(DataLoader):
    def __init__(self, dataset, batch_size=1, shuffle=True, sampler=None,
                 batch_sampler=None, num_workers=0, collate_fn=None,
                 pin_memory=False, drop_last=False, timeout=0,
                 worker_init_fn=None, multiprocessing_context='fork'):
        ...
        super(CustomMetaDataLoaderForHydraMultiProcess, self).__init__(dataset, batch_size=batch_size,
...
class CustomBatchMetaDataLoaderForHydraMultiProcess(CustomMetaDataLoaderForHydraMultiProcess):
    def __init__(self, dataset, batch_size=1, shuffle=True, sampler=None, num_workers=0,
                 pin_memory=False, drop_last=False, timeout=0, worker_init_fn=None, multiprocessing_context='fork'):
        collate_fn = BatchMetaCollate(default_collate)

        super(CustomBatchMetaDataLoaderForHydraMultiProcess, self).__init__(dataset,
...
# class CustomBatchMetaDataLoaderForHydraMultiProcess(CustomMetaDataLoaderForHydraMultiProcess):
#     def __init__(self, multiprocessing_context='fork', **kwargs):

#         super(CustomBatchMetaDataLoaderForHydraMultiProcess, self).__init__(multiprocessing_context = multiprocessing_context, **kwargs)


# class CustomMetaDataLoaderForHydraMultiProcess(DataLoader):
#     def __init__(self, multiprocessing_context='fork', **kwargs):
#         super(CustomMetaDataLoaderForHydraMultiProcess, self).__init__(multiprocessing_context=multiprocessing_context, **kwargs)
...
class MetaNML:
    def __init__(self, model=None, hidden_sizes=[1024, 1024], input_dim=2, num_classes=2,
                 ...
                 use_cuda=True, num_workers=8, metalearner=ModelAgnosticMetaLearning, weight_decay_lambda=0,
                 device=None, **kwargs):
        """
        ...
        if dist_weight_thresh:
            self.kernel = None  # KernelEmbedding(dist_weight_thresh, self.model)
        else:
            self.kernel = None

        # Initialize MAML and the meta-optimizer
        self.meta_optimizer = torch.optim.Adam(self.model.parameters(), lr=meta_lr)
        self.device = device  # torch.device('cuda' if use_cuda and torch.cuda.is_available() else 'cpu')
    ...
    def set_fixed_dataset(self, train_data, test_data,
                          task_batch_size=0, test_strategy='sample', test_batch_size=128, include_query_point=True,
                          mixup_alpha=None):
        """
    ...
    def train(self, train_data=None, test_data=None, batch_size=0, accumulation_steps=1, num_epochs=1,
              test_strategy='all',
        ...
        if self._use_fixed_dataset:
            ...
            return self.do_train(tasks, self._fixed_test_data, batch_size=self._fixed_task_batch_size,
                                 num_epochs=num_epochs,
                                 test_strategy=self._fixed_test_strategy, test_batch_size=self._fixed_test_batch_size,
                                 include_query_point=self._fixed_include_query_point,
                                 mixup_alpha=self._fixed_mixup_alpha, verbose=verbose)
        else:
            assert (
                        train_data is not None and test_data is not None), "Must provide train_data and test_data when not in fixed dataset mode"

            if reweight_to_uniform:
                e_Phi = np.sum(
                    self.evaluate(train_data[0], num_grad_steps=self.num_adaptation_steps, train_data=test_data,
                                  normalize=False), axis=1)
                weights = np.clips((e_Phi - 1) / (2 - e_Phi), 0, None)
                idxs = np.random.choice(range(len(train_data[0])), len(train_data[0]), replace=True,
            ...
            results = self.do_train(train_data, test_data, batch_size=batch_size, accumulation_steps=accumulation_steps,
                                    num_epochs=num_epochs,
                                    test_strategy=test_strategy, test_batch_size=test_batch_size,
                                    include_query_point=include_query_point,
    ...
    def train_embedding(self, train_data, num_epochs=800, verbose=True):
        if self.train_vae:
            losses, nlls, klds = train_model_vae(self.model_vae, data=train_data, num_epochs=num_epochs,
                                                 verbose=verbose)
    ...
    def do_train(self, train_data, test_data, batch_size=0, accumulation_steps=1, num_epochs=1, test_strategy='all',
        ...
        for _ in range(num_epochs):
            ...
            trainloader = CustomBatchMetaDataLoaderForHydraMultiProcess(dataset,
                                                                        batch_size=batch_size, shuffle=True,
                                                                        num_workers=self.num_workers, pin_memory=True)

            num_batches = ceil(dataset.num_tasks / batch_size)
            results = self.metalearner.train(trainloader, accumulation_steps=accumulation_steps,
                                             max_batches=num_batches, is_classification_task=True,
    ...
    def evaluate(self, inputs, normalize=True, num_grad_steps=None, step_size=None, train_data=None,
        ...
        for x in tqdm(inputs, disable=not verbose):
            results.append(
                self._get_metanml_probs(x, normalize=normalize, num_grad_steps=num_grad_steps, step_size=step_size,
