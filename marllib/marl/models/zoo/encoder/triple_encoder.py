...
class TripleHeadEncoder(nn.Module):
    ...
    def __init__(self, custom_model_config, model_arch_args, obs_space):
        ...
        status_grid_space = spaces.Dict({
            # 'agents_state': spaces.Box(low=0, high=2, shape=(self.custom_config['status_dim'],), dtype=np.float32),
            'obs': spaces.Box(low=0, high=1, shape=eval(self.custom_config['grid_dim'], ), dtype=np.float32)}
        )
    ...
    def forward(self, inputs):
        ...
        output = self.status_grid_encoder(inputs[Constants.IMAGE_STATE])
        if self.emergency_encoder_arch == 'mlp':
