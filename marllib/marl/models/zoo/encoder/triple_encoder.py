...
class TripleHeadEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        selection = nn.functional.softmax(self.emergency_selector(emergency_embedding), dim=1)
        selected_emergency = emergency.reshape(selection.shape[0], -1, 2)[torch.arange(0, selection.shape[0]),
        torch.argmax(selection, dim=1)]
        executor_obs = status.clone()
        executor_obs[..., -self.emergency_dim: -self.emergency_dim + 2] = selected_emergency
